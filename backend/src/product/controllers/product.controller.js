...
export const getAllProducts = async (req, res, next) => {
  ...
  try {
    ...
    for (const key in req.query) {
      if (key === "keyword" || key === "page" || key === "limit") {
        continue;
      }
      ...
      if (
        ...
        for (const op in value) {
          filters[key][`$${op}`] = Number(value[op]);
        }
      } else {
        filters[key] = value;
      }
    ...
    if (keyword) {
      finalQuery.name = new RegExp(keyword, "i");
    }

    // console.log("FINAL QUERY =", finalQuery);
    const products = await getAllProductsRepo(finalQuery, skip, itemsPerPage);
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    if (req.user._id != reviewToBeDeleted.user) {
      return next(
